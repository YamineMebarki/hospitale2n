                        ...
                        if ($newsPatient->mail == $mail2) {
                            ...
                            if (filter_var($newsPatient->mail, FILTER_VALIDATE_EMAIL)) {
                                ...
                                if ($newsPatient->verifyMail() == 0) {
                                    ...
                                    $arrayPatient = $newsPatient->getLastId();
                                    $result = end($arrayPatient);
                                    $appointement = new appointments();
                                    $appointement->dateHour = secur($_POST['date'].' '.$_POST['time'] );
                                    $appointement->id = end($result);
                                    if ($appointement->verifyAppointments() == 0) {
                                        $appointement->insertNewAppointments();
                                        header('location:index.php?list_patients&page=1');
                                    }else{
